<template>
  ...
  <div>
    ...
    <div>
      <button type="submit" @click="downloadFile">
        下载文件
      </button>
    </div>
  ...
  <div>
    ...
    <div v-if="fileUrl" class="file-display-container">
      <!-- 图片展示 -->
      <img v-if="fileType.startsWith('image/')" :src="fileUrl" alt="展示图片" class="display-image">
      ...
      <div v-else-if="fileType.includes('powerpoint') || fileType.includes('pptx') || fileType.includes('ppt')" class="ppt-container">
...
import { addMaterial } from '@/api/modules/teaching/MaterialAPI'
...
const downloadFile = async () => {
  ...
  try {
    const response = await axios.get('http://localhost:80/api/teaching/file/download?fileName=' + downloadFileName.value, {
      responseType: 'blob'
    })
...
const displayFile = async () => {
  ...
  try {
    ...
    const response = await axios.get('http://localhost:80/api/teaching/file/download?fileName=' + displayFileName.value, {
      responseType: 'blob'
    })

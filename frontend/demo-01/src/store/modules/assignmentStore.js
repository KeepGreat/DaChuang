...
export const useAssignmentStore = defineStore('assignment', {
  ...
  getters: {
    // 获取某个课程的作业列表
    getAssignmentsByCourseId: (state) => {
      return (courseId) => state.assignments.filter(assignment => assignment.courseId === courseId)
    },

    // 获取作业总数
    assignmentCount: (state) => state.assignments.length,

    // 根据ID获取作业
    getAssignmentById: (state) => {
      return (assignmentId) => state.assignments.find(assignment => assignment.id === assignmentId)
    },

    // 获取作业统计信息
    getAssignmentStats: (state) => (courseId) => {
  ...
  actions: {
    ...
    // 更新作业状态
    updateAssignmentStatus(assignmentId, status, score = null) {
    ...
    addSubmissionHistory(assignmentId, submission) {
      if (!this.submissionHistory[assignmentId]) {
        this.submissionHistory[assignmentId] = []
      }
      this.submissionHistory[assignmentId].unshift(submission)
    },

    // 获取作业的提交历史
    getSubmissionHistory(assignmentId) {
      return this.submissionHistory[assignmentId] || []
    },

    // 获取作业的代码模板
    getCodeTemplate(assignmentId, language) {
      const templates = {
...
public class Main {
    ...
    // 提交作业
    async submitAssignment(assignmentId, code, language) {
      this.loading = true
      try {
        // 模拟提交过程
        ...
        // 添加提交历史
        this.addSubmissionHistory(assignmentId, submission)

        // 更新作业状态
        this.updateAssignmentStatus(assignmentId, '已提交')
